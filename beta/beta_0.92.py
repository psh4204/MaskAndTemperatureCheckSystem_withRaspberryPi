...
class DataController:
    ...
    def checkTargetTemp(self, temp):
        ...
        print(temp, self.targetTemp)
        if temp >= 34:
            self.tempWeight = temp/11.7
            self.targetTemp = round(32 + self.tempWeight, 1)
        elif (not(4 < int(time.strftime('%m'))<10)) and (10 < temp <30) :
            self.tempWeight = temp/6
            self.targetTemp = round(32 + self.tempWeight, 1)
        elif temp < 34:
    ...
    def writeAreaContents(self, input_area, input_place):
...
def showImage(videostream, dataCtr):
    global window, current_place, now, current_time, season_w
    ...
    if realTempCheck > season_w:
        ...
        if motion:
            ...
            if (scores[maxScoreIndex] > min_conf_threshold):
                ...
                # 현재 기기의 정보를 보낸다.
                dataCtr.writeAreaContents(current_local, current_place)
